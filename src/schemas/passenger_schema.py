...
NAME_METADATA = metadata = {
    'example': 'Snyder, Mrs. John Pillsbury (Nelle Stevenson)'}
...
class PassengerSchema(Schema):
    ...
    Attributes:
        ...
        ticket_class (int): The passenger's ticket class (1, 2 or 3).
        sex (str): The passenger's gender (male or female).
        age (float): The passenger's age (min 0, max 120 years of age).
        number_siblings_spouses (int): The number of siblings and/or
        spouses related to the passenger aboard of the Titanic (min 0).
        number_parents_children (int): The number of parents and/or
        children related to the passenger aboard of the Titanic (min 0).
        ticket (str): The number of the passenger's ticket.
        fare (float): The price of the passenger's ticket (min 0).
        cabin (str): The ticket's designated cabin (must be a letter followed by two numbers).
        embarked (str): The passenger's boarding port (Cherbourg, Queenstown or Southampton).
    """

    name = fields.Str(required=True, metadata=NAME_METADATA,
                      description=NAME_DESCRIPTION, validate=validate.Length(min=3, max=100,
                                                                             error="Name length must be between 3 and 100 characters."))
    ticket_class = fields.Integer(required=True, metadata=TICKET_CLASS_METADATA,
                                  description=TICKET_CLASS_DESCRIPTION, validate=validate.OneOf(
                                      [1, 2, 3],
                                      error="Ticket class must be 1 (First), "
                                      "2 (Second), or 3 (Third)"))
    ...
    embarked = fields.Str(
        ...
        validate=validate.OneOf(['Cherbourg', 'Queenstown', 'Southampton'],
                                error="Embkarked value must be one either Cherbourg, Queenstown or Southampton."))
...
class PassengerViewSchema(Schema):
    ...
    cabin = fields.Str(
        required=False, allow_none=True, metadata=CABIN_METADATA, description=CABIN_DESCRIPTION)
    embarked = fields.Str(
        required=False, allow_none=True, metadata=EMBARKED_METADATA, description=EMBARKED_DESCRIPTION)
